...
#endif
	if (currentConfig.volume < 0 || currentConfig.volume > 99) currentConfig.volume = 99;
#ifdef __GP2X__
...
char *emu_GetSaveFName(int load, int is_sram, int slot)
{
	...
	if (is_sram)
	{
		...
		sprintf(saveFname, "%s/Media/ROMs/GENESIS/%s", appworkdir, saveFname);
		if (load) {
...
int emu_SaveLoadGame(int load, int sram)
{
	...
	sprintf(saveFname, "%s%s", romFileName, (PicoMCD&1) ? ".brm" : ".srm");
#endif
