	...
	[ _files removeAllObjects ];
        ...
        if (_recent == NO && _bookmarks == NO) {
            ...
            while ((file = [dirEnum nextObject])) {
                ...
                if (_saved) {
                        if (!strcasecmp(fn + (strlen(fn)-7), ".mds.gz")) 
                            [_files addObject: file];
                } else {
                        if (!strcasecmp(fn + (strlen(fn)-4), ".zip"))
                            [_files addObject: file];
                        if (!strcasecmp(fn + (strlen(fn)-4), ".smd"))
                            [_files addObject: file];
                        if (!strcasecmp(fn + (strlen(fn)-4), ".iso"))
                            [_files addObject: file];
                        if (!strcasecmp(fn + (strlen(fn)-4), ".mds"))
                            [_files addObject: file];
                        if (!strcasecmp(fn + (strlen(fn)-4), ".bin"))
                            [_files addObject: file];
                        if (!strcasecmp(fn + (strlen(fn)-4), ".gen"))
                            [_files addObject: file];                            
                }
            }
            ...
            FILE *file;
            if (_recent == YES) {
                file = fopen_home("Library/Preferences/genesis4iphone.history", "r");
            } else {
                file = fopen_home("Library/Preferences/genesis4iphone.bookmarks", "r");
            }
            if (file) {
                char buff[1024];
