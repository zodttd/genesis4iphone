...
- (void)_willBeDeleted
{
    ...
    if ((!strcmp(fn + (strlen(fn)-7), ".mds.gz"))
     	|| (!strcasecmp(fn + (strlen(fn)-4), ".zip"))
     	|| (!strcasecmp(fn + (strlen(fn)-4), ".smd"))
     	|| (!strcasecmp(fn + (strlen(fn)-4), ".mds"))
     	|| (!strcasecmp(fn + (strlen(fn)-4), ".bin"))
     	|| (!strcasecmp(fn + (strlen(fn)-4), ".gen"))
...
- (void)deleteBookmark: (const char *)file {
    ...
    out = fopen("/tmp/genesis4iphone.bookmarks", "w");
    if (out) {
        char *s, *t, *u;
        t = strdup(file);
        s = strtok(t, "/");
        ...
        fclose(out);
        free(t);
        rename("/tmp/genesis4iphone.bookmarks", "/var/mobile/Library/Preferences/genesis4iphone.bookmarks");
        rename("/tmp/genesis4iphone.bookmarks", "/var/root/Library/Preferences/genesis4iphone.bookmarks");
    }
}
