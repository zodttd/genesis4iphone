...
- (void)alertSheet:(UIAlertSheet *)sheet buttonClicked:(int)button {
    ...
    if (sheet == badROMSheet) {
        ...
        if (button == 1) 
        {
			if( (!strcasecmp(__lastfileName + (strlen(__lastfileName)-7), ".mds.gz")) )
			{
        ...
        switch (button) {
            ...
            case (2):
                ...
                if ( (!strcasecmp(tempbuf + 
                        (strlen(tempbuf)-7), ".mds.gz")) )
                {
                	sprintf(tempbuf, "/var/root/Media/ROMs/GENESIS/%s", [ m_currentFile cStringUsingEncoding: NSASCIIStringEncoding ]);
                    unlink(tempbuf);
                	sprintf(tempbuf, "/var/mobile/Media/ROMs/GENESIS/%s", [ m_currentFile cStringUsingEncoding: NSASCIIStringEncoding ]);
                    unlink(tempbuf);
                    [ savedBrowser reloadData ];
                } else {
            ...
            switch (currentView) {
                ...
                case CUR_BROWSER:
                    if (currentBrowserPage == CB_RECENT) {
                        unlink("/var/root/Library/Preferences/genesis4iphone.history");
                        unlink("/var/mobile/Library/Preferences/genesis4iphone.history");
                        [ recentBrowser reloadData ];
                    }
                    break;
...
- (BOOL)isBookmarked: (NSString *)file {
    ...
    in = fopen_home("Library/Preferences/genesis4iphone.bookmarks", "r");
    if (in) { 
...
- (void)addBookmark: (NSString *)file {
    ...
    out = fopen_home("Library/Preferences/genesis4iphone.bookmarks", "a");
    if (out) {
        char *s, *t, *u;
        t = strdup(cFileName);
        s = strtok(t, "/");
...
- (void)load {
	...
	[ prefTable release ];
        ...
        in = fopen_home("Library/Preferences/genesis4iphone.history", "r");
        out = fopen("/tmp/genesis4iphone.history", "w");
        if (out) {
            char *s, *t, *u;
            t = strdup(cFileName);
            s = strtok(t, "/");
            ...
            fclose(out);
            rename("/tmp/genesis4iphone.history", "/var/mobile/Library/Preferences/genesis4iphone.history");
            rename("/tmp/genesis4iphone.history", "/var/root/Library/Preferences/genesis4iphone.history");
            free(t);
        }
