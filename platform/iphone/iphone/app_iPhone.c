...
extern short __attribute__((aligned(4))) sndOutBuffer[2*44100/50];
extern short* sndOutPtr;
    ...
    AudioQueueBufferRef outQB)
{
	...
	if (inData->frameCount > 0) {
		...
		if( (unsigned long)sndOutPtr + inData->frameCount * 4 <= ((unsigned long)sndOutBuffer + ((2*44100/50)*2)) )
		{
			memcpy(coreAudioBuffer, sndOutPtr, inData->frameCount * 4);
		}
...
int app_OpenSound(int samples_per_sync, int sample_rate) {
   ...
   in.frameCount = 512; //512; //(1024 * (16)) / 4;
...
int app_LoadPreferences() {
    ...
    /* Load Preferences */
    f = fopen_home("Library/Preferences/genesis4iphone.v1", "rb");
    if (!f) 
        return -1;
...
int app_SavePreferences() {
    ...
    /* Load Preferences */
    f = fopen_home("Library/Preferences/genesis4iphone.v1", "wb");
    if (!f) return -1;
...
void LOGDEBUG(const char *text, ...)
{
  ...
  f = fopen("/tmp/genesis4iphone.debug", "a");
  fprintf(f, "%s\n", debug_text);
  fclose(f);
}
#endif
