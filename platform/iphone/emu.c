...
void emu_getMainDir(char *dst, int len)
{
	...
	sprintf(tempdir, "%s/Media/ROMs/GENESIS/", appworkdir);
	strncpy(dst, tempdir, len);
	len -= 32; // reserve
	if (len < 0) len = 0;
	dst[len] = 0;
...
void emu_setDefaultConfig(void)
{
	...
	currentConfig.EmuOpt  = 0x1f | 0x600 | 0x80; // | confirm_save, cd_leds
	currentConfig.PicoOpt = 0x0f /*| 0xc00 | 0x0020*/; // | use_940, cd_pcm, cd_cdda
	currentConfig.PsndRate = 44100; // 22050;
	currentConfig.PicoRegion = 0; // auto
	currentConfig.PicoAutoRgnOrder = 0x184; // US, EU, JP
	currentConfig.Frameskip = preferences.frameSkip == 5 ? -1 : preferences.frameSkip;
	...
	{
		currentConfig.EmuOpt &= ~(0x0004);
		currentConfig.PicoOpt &= ~(0x0007);
	}

	// Temp disable framerate counter
	//currentConfig.EmuOpt &= ~(2);
}
...
static void cd_leds(void)
{
	...
	p = (unsigned int *)((short *)gp2x_screen + 320*2+4+2);
	col_g = (old_reg & 2) ? 0x06000600 : 0;
	col_r = (old_reg & 1) ? 0xc000c000 : 0;
	*p++ = col_g; *p++ = col_g; p+=2; *p++ = col_r; *p++ = col_r; p += 320/2 - 12/2 + 1;
	*p++ = col_g; p+=3; *p++ = col_r; p += 320/2 - 10/2;
	*p++ = col_g; *p++ = col_g; p+=2; *p++ = col_r; *p++ = col_r;
}
...
static void blit(const char *fps, const char *notice)
{
	...
	if (PicoOpt&0x10)
	{
		...
		vidCpy8to16((unsigned short *)gp2x_screen+320*8, PicoDraw2FB+328*8, localPal, lines_flags);
	}
...
{
	if (full) memset32(gp2x_screen, 0, 320*240*2/4);
	else      memset32((int *)((char *)gp2x_screen + 320*232*2), 0, 320*8*2/4);
}
...
static void updateKeys(void)
{
	...
	{
		if (keys & (1 << i)) {
			unsigned long pl, acts = currentConfig.KeyBinds[i];
			if (!acts) continue;
			pl = (acts >> 16) & 1;
			if (combo_keys & (1 << i)) {
				unsigned long u = i+1, acts_c = acts & combo_acts;
...
#else
	PicoPad[0] = (unsigned short) keys;
	PicoPad[1] = (unsigned short) 0;
#endif
